...
def train(net, rd_lambda, train_iter, num_epochs, loss, device, \
    ...
    logging.basicConfig(filename="./train_info"+str(rd_lambda)+".log", level=logging.INFO)
    logging.getLogger()
    ...
    for epoch in range(num_epochs):
        ...
        # count = 1
        step_for_save += 1
        if step_for_save % 100 == 0:
            # 每一百个epoch保存一次模型
            torch.save(net.state_dict(), os.path.join(os.getcwd(), "model_param", "model"+str(rd_lambda)))
...
def test(net, rd_lambda, test_iter, loss, device):
    logging.basicConfig(filename="./train_info"+str(rd_lambda)+".log", level=logging.INFO)
    logging.getLogger()
    path = 'image/test_results'+str(rd_lambda)
    ...
    for num, X in enumerate(test_iter):
        ...
        #     print(X_hat[i] * 255, X[i] * 255)
        #     img = toPIL(X_hat[i] * 255)
        #     img.save(os.path.join(os.getcwd(), 'image/train_results'+str(rd_lambda), str(epoch)+'-'+str(count)+'-'+str(i)+'.bmp'))                  
        for i in range(X_hat.shape[0]):
            img = toPIL(X_hat[i].clamp(0., 1.))
            img.save(os.path.join(os.getcwd(), 'image', 'test_results'+str(rd_lambda),
                                    str(num) + '-' + str(i) + '.bmp'))
...
if __name__ == "__main__":
    ...
    train_iter = dataLoader("/home/u200111522/jupyterlab/train_image/train2017", batch_size=8)
    ...
    #net.load_state_dict(torch.load('model_param/model128'))
    train(net=net, rd_lambda=2048, train_iter=train_iter, num_epochs=400, loss=loss, device='cuda:0', log_interval=10, update_interval=2)
    test(net=net, rd_lambda=2048, test_iter=test_iter, loss=loss, device='cuda:0')
